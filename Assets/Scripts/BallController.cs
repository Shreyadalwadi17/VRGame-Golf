...
public class BallController : MonoBehaviour {
    ...
    public float baseForceAmount = 1000f;
    public float maxForceAmount = 2000f;
    public float minForceAmount = 500f;
    ...
    void OnTriggerEnter (Collider other) {
        ...
        if (other.gameObject.CompareTag ("Club") && !hasShooted) {
            ...
            float currentSpeed = other.GetComponent<Rigidbody>().velocity.magnitude;


            float scaledForce = Mathf.Lerp(maxForceAmount, minForceAmount, currentSpeed / maxSpeed);
            float finalForce = Mathf.Clamp(scaledForce, minForceAmount, maxForceAmount);


            other.GetComponent<Rigidbody>().AddForce(transform.forward * finalForce);
